...
import createProduct from "../assets/modification .png"
...
const EditProductPage = () => {
    ...
    const { productUpdateLoading, productUpdateRequest, singleProductRequest, singleProduct } = productStore();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const result = await productUpdateRequest({
            data: {
                title: title,
                sub_title: sub_title,
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                await getAllCategoryRequest(1000, 1);
                await getBrandRequest(1000, 1);
    ...
    return (
        <div className="bg-gray-200 pb-30 w-auto min-h-screen flex flex-col items-center justify-center">

            <div className="bg-white w-250 max-[849px]:w-150 max-[449px]:w-80 h-500 max-[849px]:h-700 max-[449px]:h-820 rounded-2xl shadow shadow-black/30 my-20 py-10 px-10">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {/* 1st section */}
                    <div className="flex flex-row max-[850px]:flex-col gap-10">
                        {/* Img section */}
                        <div>
                            ...
                            <div className="flex flex-col gap-2 relative mt-5">
                                ...
                                <p className="text-[10px] text-red-700">Image size Should be Square & Remove background for a cleaner and more professional product display. Only JPEG, JPG, and PNG image formats are allowed.</p>
                                <input id="big_image" onChange={(e) => setbig_image(e.target.files[0])} type="file" className="bg-black/10 h-90 hover:cursor-pointer w-110 max-[449px]:w-60 border-dotted border-black border-3  rounded-2xl pt-75 pl-30 max-[449px]:pl-6" />
                                <img className="h-10 w-10 max-[449px]:left-25 max-[449px]:top-80 absolute top-90 left-18" src={image}></img>
                                <img className="h-40 w-40 max-[449px]:h-30 max-[449px]:w-30 absolute top-30 left-35 max-[449px]:left-15" src={images}></img>
                            ...
                            {/* Short img */}
                            <div className="flex flex-row max-[449px]:grid max-[449px]:grid-cols-2 max-[449px]:w-80 max-[449px]:gap-y-5 justify-between py-10 426px max-[849px]:w-110">
                            ...
                            <div className=" flex flex-col gap-3">
                                {/* Remark & stock */}
                                <div className="flex flex-row max-[449px]:flex-col gap-3">
                                    <div className="flex flex-col gap-2">
                                        <label htmlFor="remark">Remark:</label>
                                        <input id="remark" value={remark} onChange={(e) => setremark(e.target.value)} type="text" className="border px-2 h-8 w-70 max-[449px]:w-60 border-black shadow-inner shadow-black/20 focus:outline-0 py-1 rounded-md bg-white" />
                                    </div>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <input id="title" value={title} onChange={(e) => setTitle(e.target.value)} className="h-10 max-[849px]:w-130 max-[449px]:w-60 border-black bg-white border-2 shadow-inner shadow-black/30 focus:outline-0  w-110  rounded-md px-2 py-2" type="text" />
                            </div>
                            {/* subtitle */}
