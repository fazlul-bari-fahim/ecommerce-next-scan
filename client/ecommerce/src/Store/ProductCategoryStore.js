...
const ProductCategoryStore = create((set) => ({
    ...
    navbartotalCategory: null,
    navbarallCategory: null,
    navbargetAllCategoryLoading: false,
    navbargetAllCategoryRequest: async (per_page, page_no) => {
        try {

            set({ navbargetAllCategoryLoading: true });
            const res = await axios.get(baseURL + `/all-category/${per_page}/${page_no}`, {
                withCredentials: true,
                credentials: "include",
            });
            ...
            if (res?.data?.success === true) {
                set({ navbargetAllCategoryLoading: false });
                // toast.success(res?.data?.message);
                set({ navbartotalCategory: res?.data?.data?.totalCount?.[0]?.count });
                set({ navbarallCategory: res?.data?.data?.categories });
                return true;
            } else {
                set({ navbargetAllCategoryLoading: false });
                toast.error(res?.data?.message);
